...
class TestGercherbergSaxton(unittest.TestCase):
    ...
    def test_bin_amp_phase_gercherberg_saxton(self):
        ...
        holo_wf = bin_amp_phase_gercherberg_saxton(start_wf, target_wf.amplitude, propagator, writer, max_iter=max_iter, scale_loss=scale_loss, summary_freq=summary_freq)
    ...
    def test_bin_amp_amp_gercherberg_saxton(self):
        ...
        holo_wf = bin_amp_amp_gercherberg_saxton(diffused_start_wf, target_wf.amplitude, propagator, writer, max_iter=max_iter, scale_loss=scale_loss, bin_amp_mode=bin_amp_mode, summary_freq=summary_freq)
        holo_wf.plot(intensity=defaultdict(str, title=experiment+"__holo", path=output_path, save=True))
    ...
    def test_phase_sgd(self):
        ...
        holo_wf = phase_sgd(start_wf, target_wf.amplitude, propagator, loss_fn, writer, max_iter=max_iter, lr=lr, scale_loss=scale_loss, summary_freq=summary_freq)
    ...
    def test_bin_amp_amp_sig_sgd(self):
        ...
        diffused_start_wf = opt.RandomPhaseMask().forward(start_wf)
        holo_wf = bin_amp_amp_sig_sgd(diffused_start_wf, target_wf.amplitude, propagator, loss_fn, from_amp_to_bin_amp, writer, max_iter=max_iter, lr=lr, scale_loss=scale_loss, summary_freq=summary_freq)
        holo_wf.plot(intensity=defaultdict(str, title=experiment + "__holo", path=output_path, save=True))
