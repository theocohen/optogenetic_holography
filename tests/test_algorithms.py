...
from optogenetic_holography.algorithms import bin_amp_phase_gs, bin_amp_amp_gs, \
...
if MULTIPLANE: # 3D
    ...
    # 2D
    target_wf = opt.Wavefront.from_images(input_path + "bahtinov_mask.jpg", optimize_resolution=False)
    z = 10 * opt.cm
    summary_freq=10
...
start_wf = opt.Wavefront(target_wf.resolution, roi=target_wf.roi)
...
propagator = opt.FourierFresnelPropagator(0, 0, wavelength, pixel_pitch, z)
...
class TestGercherbergSaxton(unittest.TestCase):
    ...
    def test_bin_amp_phase_gercherberg_saxton(self):
        ...
        holo_wf = bin_amp_phase_gs(start_wf, target_amp, propagator, writer, max_iter=max_iter, scale_loss=scale_loss, summary_freq=summary_freq, batch=time_averages)
    ...
    def test_bin_amp_amp_gercherberg_saxton(self):
        ...
        holo_wf = bin_amp_amp_gs(diffused_start_wf, target_amp, propagator, writer, iterations=iterations, scale_loss=scale_loss, bin_amp_mode=bin_amp_mode, summary_freq=summary_freq, batch=time_averages)
        holo_wf.plot(intensity=defaultdict(str, title=experiment+"__holo", path=output_path, save=True))
    ...
    def test_phase_sgd(self):
        ...
        holo_wf = phase_sgd(start_wf, target_amp, propagator, loss_fn, writer, iterations=iterations, lr=lr, scale_loss=scale_loss, summary_freq=summary_freq, batch=time_averages)
    ...
    def test_bin_amp_phase_sgd(self):
        ...
        holo_wf = bin_amp_phase_sgd(start_wf, target_amp, propagator, loss_fn, writer, iterations=iterations, lr=lr, scale_loss=scale_loss, summary_freq=summary_freq)
        holo_wf.plot(intensity=defaultdict(str, title=experiment + "__holo", path=output_path, save=True))
    ...
    def test_bin_amp_amp_sgd(self):
        ...
        holo_wf = bin_amp_amp_sgd(diffused_start_wf, target_amp, propagator, loss_fn, writer, iterations=iterations, lr=lr, scale_loss=scale_loss, bin_amp_mode=bin_amp_mode, summary_freq=summary_freq, batch=time_averages)
        holo_wf.plot(intensity=defaultdict(str, title=experiment + "__holo", path=output_path, save=True, ))
    ...
    def test_bin_amp_amp_sig_sgd(self):
        ...
        holo_wf = bin_amp_amp_sig_sgd(diffused_start_wf, target_amp, propagator, loss_fn, writer, iterations=iterations, lr=lr, scale_loss=scale_loss, summary_freq=summary_freq, batch=time_averages)
        holo_wf.plot(intensity=defaultdict(str, title=experiment + "__holo", path=output_path, save=True))
